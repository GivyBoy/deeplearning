...
class Parameters:
    def __init__(self, experiment, file):
        ...
        self.train_data_dir = (
            "/mnt/c/Users/givan/Desktop/deeplearning/datasets/segmentation/" + self.experiment + "images/"
        )
        self.train_mask_dir = (
            "/mnt/c/Users/givan/Desktop/deeplearning/datasets/segmentation/" + self.experiment + "masks/"
        )
...
class MAIN:
    def __init__(self, experiment, model, file="unet_f/"):
        self.params = Parameters(experiment=experiment, file=file)
        ...
        self.model = model(in_channels=self.hyperparams.IN_CHANNELS, num_classes=self.hyperparams.NUM_CLASSES)
        ...
        #     shape=self.hyperparams.RESIZE_SHAPE, transform=self.params.TRANSFORM, mode=self.hyperparams.TRANSFORM_MODE
        # )
        ...
        self.dataset = DATASET(self.params.train_data_dir, self.params.train_mask_dir, self.params.experiment)

        # self.writer = TensorboardWriter(
        #     exp=self.params.experiment, PATH=self.params.LOGDIR, fig_path=self.params.FIG_PATH, num_data=48
        # )

        self.trainer = MainTrainer(
            model=self.model,
            params=self.params,
            ...
            # writer=self.writer if self.params.TO_TENSORBOARD else None,
        )
...
if __name__ == "__main__":
    trainer = MAIN(experiment="Kaggle/", model=UNet)
    trainer.run()
    # print(trainer.total_params)
