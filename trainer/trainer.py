...
class MainTrainer:
    def __init__(
        ...
        self.metric_keys = ["loss"] + [metric.__class__.__name__.lower() for metric in self.metrics]
    ...
    def fit(self):
        ...
        for epoch in range(self.epoch, self.hyperparams.NUM_EPOCHS):
            self.epoch = epoch
            self.loop = tqdm(enumerate(self.train_loader), total=len(self.train_loader), leave=False)

            epoch_training_results = self.fit_one_epoch()
            self.store(epoch_training_results, "train")
            self.print_epoch("train")

            if self.params.VALIDATION:
                epoch_val_results = self.validate()
                self.store(epoch_val_results, "test")
                ...
                if self.params.TO_TENSORBOARD:
                    self.writer.write_images(
                        keys=["Data", "Target", "Prediction"],
                        data=[self.images, self.targets, self.predictions],
                        step=epoch + 1,
    ...
    def fit_one_epoch(self):
        ...
        for _, (data, target) in self.loop:
            ...
            batch_results = torch.cat(
                (
                    torch.tensor(batch_loss.detach().cpu()).reshape(
                        1,
                    ),
                    batch_metrics,
                ),
    ...
    def fit_one_batch(self, data, target, grad=True):
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for _, (data, target) in enumerate(self.test_loader):
                ...
                val_batch_results = torch.cat(
                    (
                        torch.tensor(val_batch_loss.detach().cpu()).reshape(
                            1,
                        ),
                        val_batch_metrics,
                    ),
    ...
    def store(self, epoch_results, flag):
    ...
    def round_metrics(self, result, dec):
        return torch.round(result * 10**dec) / (10**dec)

    def print_batch(self, results):
        print_dict = {
            key: self.round_metrics(results[i].detach().cpu(), 4).numpy()
    ...
    def print_epoch(self, stage):
        if stage == "train":
            print_dict = {key: round(float(self.train_history[key][-1].detach().cpu()), 4) for key in self.metric_keys}
            print(f"\n" "Training -- Epoch:" + repr(self.epoch + 1) + " --> " + str(print_dict))
            # with open(os.path.join(self.params.result_HISTORYPATH, "history.txt"), "a") as f:
            #     f.write(f"\n" "Training -- Epoch:" + repr(self.epoch + 1) + " --> " + str(print_dict))

        elif stage == "test":
            print_dict = {key: round(float(self.test_history[key][-1].detach().cpu()), 4) for key in self.metric_keys}
            ...
            if self.test_history[self.params.METRIC_CONDITION][-1] > self.best_metric:
                cprint(
                    f"Best: {round(float(self.test_history[self.params.METRIC_CONDITION][-1]), 4)} --> "
    ...
    def update_best_metric(self, condition="max"):
        if condition == "max":
            self.best_metric = np.max((self.best_metric, self.test_history[self.params.METRIC_CONDITION][-1]))
        elif condition == "min":
            self.best_metric = np.min((self.best_metric, self.test_history[self.params.METRIC_CONDITION][-1]))
        else:
            raise Exception("Best metric update condition must be either min or max.")

    def save_results(self):
        np.save(os.path.join(self.params.result_SAVEPATH, "train_results.npy"), self.train_history)
        np.save(os.path.join(self.params.result_SAVEPATH, "test_results.npy"), self.test_history)
    ...
    def update_model(self):
        if self.BEST:
            torch.save(
                ...
                os.path.join(self.params.model_SAVEPATH, "model.pth"),
            )

    def load_model(self, final_model=False):
        checkpoint = torch.load(os.path.join(self.params.model_LOADPATH, "model.pth"), map_location=self.DEVICE)
